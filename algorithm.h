...
void stableMatching(UNORDERWSMAP studentList, UNORDERWSMAP schoolList, UNORDERWSMAP& output)
{
	...
	while (!noSize)
	{
			...
			for (auto& it : studentList)
			{
				...
				if (studentMatching[student].empty())
				{
					while (it.second.size())		//學生志願序中的學校
					{
						...
						{
							if (find(schoolList[school].begin(), schoolList[school].end(), student) != schoolList[school].end())  ///學校清單找的到學生
							{
		...
		for (auto& entry : studentMatching)
		{
			...
			{
				schoolList[entry.second].erase(find(schoolList[entry.second].begin(), schoolList[entry.second].end(), entry.first));	///把已經配對到的學生從學校列表中刪除
